...
uint8_t rx7;
uint8_t rx8;
volatile uint8_t uart7_received = 0;
volatile uint8_t uart8_received = 0;
...
//  MX_USB_OTG_FS_USB_Init();
  ...
  /* START RX HERE — after init */
    HAL_UART_Receive_IT(&huart7, &rx7, 1);
    HAL_UART_Receive_IT(&huart8, &rx8, 1);
  /* USER CODE BEGIN 2 */
//  run_interface();
  /* USER CODE END 2 */
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  uint8_t msg;
		  ...
		  if (uart7_received || uart8_received)
		  {
			  msg = '2';
			  uart7_received = 0;
			  uart8_received = 0;
		  ...
		  HAL_UART_Transmit(&huart7, &msg, 1, HAL_MAX_DELAY);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        uart7_received = 1;
        HAL_UART_Receive_IT(&huart7, &rx7, 1);   // re-arm RX
    }

    if (huart->Instance == UART8)
    {
        uart8_received = 1;
        HAL_UART_Receive_IT(&huart8, &rx8, 1);   // re-arm RX
    }
}
